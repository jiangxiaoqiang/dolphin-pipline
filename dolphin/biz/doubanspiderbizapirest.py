...
class doubanspiderbizapirest:
    ...
    def get_douban_book_id_by_restservice(self):
        url = confighelper.getValue(self, 'global', 'rest_service_address') + "/spider/api/v1/doubanbook"
        return dolphinhttpclient.get(dolphinhttpclient,url)

    def save_douban_book_by_restservice(self,book):
        url = confighelper.getValue(self,'global','rest_service_address') + "/spider/api/v1/book"
        return dolphinhttpclient.post_json(dolphinhttpclient,url,book) 
    ...
    def save_rest_api_book_lists(self):
        ...
        if book['isbn13'] is not None and book['isbn13'] != '':
            book.creator = confighelper.getGlobalValue(self,'spider_man')
